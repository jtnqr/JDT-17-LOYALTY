...
graph TD
    subgraph "Authentication & Authorization"
        A1[User / Admin] -->|Login Credentials| A2(Auth Service)
        A2 -->|Validate & Generate| A3[JWT Token]
    end

    subgraph "Admin Flow"
        B1[Admin] -->|POST /partners with JWT| B2(Add Partner API)
        B2 -->|Validate Admin Role| B3[(Database)]
        B3 -->|Insert PARTNER record| B4[Initialize 0 Balance for existing members]
    end
    ...
    subgraph "Background Job (System)"
        D1[Scheduler / Cron Job] -->|Trigger Daily| D2(Point Expiry Job)
        D2 -->|Query EARN transactions past expiresAt| D3[(Database)]
        D3 -->|Calculate unexpired points| D4[Insert EXPIRED Transaction]
